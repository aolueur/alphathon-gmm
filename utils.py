...
class StockDataProcessor:
    ...
    def filter_stocks_before_cutoff(self):
        ...
        stocks = stocks[stocks['Date added'] <=
                        self.start_date].reset_index(drop=True)
    ...
    def download_price_data_and_calculate_log_returns(self, stocks):
        ...
        print(
            f"Downloading price data for {len(assets)} stocks from Yahoo Finance...")

        price_data = yf.download(assets, start=self.start_date, end=self.end_date)[
            'Adj Close']
        ...
        stocks_log_ret = (np.log(price_data) -
                          np.log(price_data.shift(1))) * 100
        stocks_log_ret = stocks_log_ret.dropna()

        # Save log returns data
        log_ret_output_file = os.path.join(
            self.output_folder, 'stocks_log_return.csv')
